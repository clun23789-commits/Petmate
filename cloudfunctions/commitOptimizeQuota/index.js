...
function fail(error) {
  console.error("commitOptimizeQuota failed", error);
  return {
    ok: false,
    errorCode: "OPTIMIZE_QUOTA_COMMIT_FAILED",
...
exports.main = async (event = {}) => {
  try {
    ...
    if (!OPENID || !reservationId) {
      return expectedError("OPTIMIZE_QUOTA_COMMIT_INVALID_PAYLOAD", "优化预占信息缺失，无法确认扣减");
    }
    ...
    const nextUsedCount = normalizeNumber(quotaDoc.usedCount) + 1;
