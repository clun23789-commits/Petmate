...
- 新任务通过 `ownerOpenid + operationType + clientRequestId` 的确定性 `_id` 幂等创建
- 按确定性 `_id` 查询单个新任务，并通过现有索引兼容历史查询
...
- 确定性任务 `_id` 是新任务的并发唯一约束；`idx_owner_task` 继续保留给历史任务和运维查询，不应误改为唯一索引。
- `works / workVersions` 的生成结果新文档也使用确定性 `_id`；现有 owner/work、owner/version 索引用于查找并更新 legacy 文档。
