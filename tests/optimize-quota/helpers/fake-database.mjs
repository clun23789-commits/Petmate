...
class QueryRef {
  constructor(database, store, collectionName, criteria = {}) {
  ...
  where(criteria) {
    return new QueryRef(this.database, this.store, this.collectionName, criteria);
  }
  ...
  async get() {
    ...
    return {
      data: Array.from(collection.values()).filter((doc) => matches(doc, this.criteria)).slice(0, this.max).map(clone)
    };
  }
...
export class FakeDatabase {
  ...
  consumeFailure(collectionName, operation) {
    const index = this.failures.findIndex((item) => item.collectionName === collectionName && item.operation === operation);
    if (index === -1) {
      return;
    }
  ...
  async runTransaction(callback) {
    const run = async () => {
      ...
      const transaction = {
        collection: (name) => new QueryRef(this, draft, name)
      };
